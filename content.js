...
var findSynonym = function findSynonym(str, difficulty){
    ...
    var found = false;
    if((freq_list[word] == undefined || freq_list[word] < 40000 - difficulty * 400) && mapObj[str] == undefined){
	...
	$.getJSON("https://words.bighugelabs.com/api/2/" + api_key + "/" + word + "/json", function(data){
		...
		for(var syn in synonyms){
		    //checks if the word is simpler than the word if a synonynm
		    if((freq_list[synonyms[syn]] > freq_list[word]) || freq_list[word] == undefined && !found){
			found = true;
